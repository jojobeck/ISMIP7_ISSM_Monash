function md = hist_run_tune_MRI_ESM(steps, loadonly)
% Historical validation run 1995-2020 using MRI-ESM2-0 forcing.
% Compares modelled mass loss (WAIS / EAIS / Peninsula) against Otosaka et al.
%
% Forcing strategy:
%   Ocean TF  : MRI-ESM2-0 historical 1995-2014 (raw_data/.../historical/ocean/tf/v3)
%               No SSP126 ocean TF found -> 2014 TF repeated for 2015-2020 [FLAGGED]
%   SMB       : MRI-ESM2-0 anomaly + RACMO 1995-2014 climatology
...
%
% Step map:
%   1  MRI_TF_hist        build annual TF mat 1995-2020 (2015-2020 = 2014 repeated)
%   2  MRI_SMB_hist       build annual SMB mat (MRI anomaly + RACMO clim, SMBgradients)
%   3  Greene_levelset     build spclevelset time series from icemask_greene
%   4  Relaxed_MRI_ESM2  short 1-year run (1995->1996) under real MRI_ESM2
...
% TODO (step 7): confirm that regions in sectors_8km.nc are 1=WAIS 2=EAIS 3=Peninsula
    ...
    org = organizer('repository', './Models_MRIESM2/', 'prefix', 'AIS_ISMIP7_', ...
    ...
    raw_hist  = './../raw_data/ISMIP7/AIS/MRI-ESM2-0/historical';
    raw_ssp   = './../raw_data/ISMIP7/AIS/MRI-ESM2-0/ssp126';
    ...
    preproc_clim  = './../preprocessed_data/Atmosphere/Clim/MRI-ESM2-0/';
   ...
   region_mask_file = [preproc_ocean 'Basins/HistRun_Regions.mat'];
    ...
    if perform(org, 'MRI_TF_hist') % {{{
        ...
        tf_files = {
            [tf_dir 'tf_AIS_MRI-ESM2-0_historical_ocean_v3_1990-1999.nc'], ...
            [tf_dir 'tf_AIS_MRI-ESM2-0_historical_ocean_v3_2000-2009.nc'], ...
            [tf_dir 'tf_AIS_MRI-ESM2-0_historical_ocean_v3_2010-2014.nc']  ...
        };
        ...
        save([preproc_front 'MRI_ESM2_TF_' num2str(start_year) '_' num2str(end_year) '.mat'], ...
             'tf_hist', 'z_data', 't_vec', '-v7.3');
        fprintf('Saved TF: %d years, %d depths.\n', nyears, nDepths);
    end % }}}

    % ================================================================= Step 2
    if perform(org, 'MRI_SMB_hist') % {{{
        ...
        %
        % MRI acabf source files are MONTHLY (12 time steps/year, confirmed via
        ...
        % MRI 1995-2014 mean on mesh [mm w.e. yr-1] for anomaly baseline
        fprintf('Computing MRI SMB mean 1995-%d...\n', hist_end);
        ...
        save([preproc_atmo 'MRI_ESM2_SMB_' num2str(start_year) '_' num2str(end_year) '.mat'], ...
             'smb_forcing', 'bgrad_forcing', 't_smb', '-v7.3');
        save([preproc_clim 'MRI_ESM2_SMB_clim_' num2str(start_year) '_' num2str(hist_end) '.mat'], ...
    ...
    % ================================================================= Step 4
    if perform(org, 'Relaxed_MRI_ESM2') % {{{
        % Short 1-year run (1995 -> 1996) using the real MRI_ESM2 ocean/SMB/
        ...
        load([preproc_front 'MRI_ESM2_TF_' num2str(start_year) '_' num2str(end_year) '.mat']);
        ...
        % --- SMB (same as HistRun) ---
        load([preproc_atmo 'MRI_ESM2_SMB_' num2str(start_year) '_' num2str(end_year) '.mat']);
        md.smb        = SMBgradients();
        ...
        md.miscellaneous.name = 'Relaxed_MRI_ESM2';
        clustername = 'gadi';
    ...
    if perform(org, 'HistRun') % {{{
        ...
        md = loadmodel(org, 'Relaxed_MRI_ESM2');
        m  = ((1+sin(71*pi/180))*ones(md.mesh.numberofvertices,1) ...
              ./ (1+sin(abs(md.mesh.lat)*pi/180)));
        md.mesh.scale_factor = (1./m).^2;
        ...
        load([preproc_atmo 'MRI_ESM2_SMB_' num2str(start_year) '_' num2str(end_year) '.mat']);
        md.smb        = SMBgradients();
        md.smb.smbref = smb_forcing;           % RACMO clim + MRI anomaly [m ice yr-1], timeseries
        ...
        md.miscellaneous.name = ['HistRun_MRI_ESM2_' num2str(start_year) '_' num2str(end_year)];
        clustername = 'gadi';
        md.cluster  = set_cluster(clustername);
        md.settings.waitonlock = 0;
...
% end % }}}
    ...
    if perform(org, 'HistRun_Validation') % {{{
        ...
        title('AIS total SMB: model output vs. MRI+RACMO forcing');
    ...
    if perform(org, 'HistRun_Assessment') % {{{
        ...
        assess_vs_otosaka(md, region_mask_file, start_year, ...
            './Data/Tables/HistRun_otosaka_assessment_MRI_ESM2.csv', ...
    ...
    if perform(org, 'HistRun_CorrectSMB') % {{{
        ...
        % from step 8's Otosaka assessment. The MRI year-to-year anomaly is
        ...
        load([preproc_clim 'MRI_ESM2_SMB_clim_' num2str(start_year) '_' num2str(hist_end) '.mat'], 'smb_racmo');

        load([preproc_ocean 'Basins/HistRun_Regions.mat'], 'reg_vert', 'region_names', 'region_ids');

        otosaka_assessment = readtable('./Data/Tables/HistRun_otosaka_assessment_MRI_ESM2.csv');
        ...
        % Save corrected forcing so hist_run_MRI_ESM2_1995_2014 and the
        ...
        save([preproc_atmo 'MRI_ESM2_SMB_corrected_' num2str(start_year) '_' num2str(end_year) '.mat'], ...
             'smb_forcing', 'bgrad_forcing', 't_smb', 'p_vert', '-v7.3');
        fprintf('Saved corrected SMB: %s\n', [preproc_atmo 'MRI_ESM2_SMB_corrected_' num2str(start_year) '_' num2str(end_year) '.mat']);

        md.smb        = SMBgradients();
        md.smb.smbref = smb_forcing_corr;      % region-corrected RACMO clim + MRI anomaly
        md.smb.b_pos  = bgrad_forcing;
        md.smb.b_neg  = bgrad_forcing;
        md.smb.href   = [md.geometry.surface ; start_year];
        ...
        md.miscellaneous.name = ['HistRun_CorrectSMB_MRI_ESM2_' num2str(start_year) '_' num2str(end_year)];
        clustername = 'gadi';
        md.cluster  = set_cluster(clustername);
        md.settings.waitonlock = 0;
        md.verbose = verbose('solution', true, 'module', true, 'convergence',true);
    ...
    if perform(org, 'HistRun_CorrectSMB_Assessment') % {{{
        ...
        assess_vs_otosaka(md, region_mask_file, start_year, ...
            './Data/Tables/HistRun_otosaka_assessment_MRI_ESM2_corrected.csv', ...
            './figures/HistRun_regional_mass_change_corrected.png', ...
            'Cumulative mass change vs Otosaka et al.  (corrected SMB, MRI-ESM2-0 hist+ssp126)');
    end % }}}
...
function nc = smb_nc_path(hist_dir, ssp_dir, hist_end, yr, var)
    if yr <= hist_end
        nc = [hist_dir sprintf('%s_AIS_MRI-ESM2-0_historical_SDBN1-2000m_v1_%d.nc', var, yr)];
    else
        nc = [ssp_dir  sprintf('%s_AIS_MRI-ESM2-0_ssp126_SDBN1-2000m_v1_%d.nc',    var, yr)];
    end
end
