...
classdef gadi
	...
	methods
		function cluster=gadi(varargin) % {{{
			...
			if (exist('gadi_settings')==2), gadi_settings; end
		...
		function disp(cluster) % {{{
			...
			%disp(sprintf('    storage: %s',cluster.executionpath));
		...
		function md = checkconsistency(cluster,md,solution,analyses) % {{{
			...
			queue_requirements_time=[24*60 48*60];
			queue_requirements_np=[480 672];
		...
		function BuildQueueScript(cluster,dirname,modelname,solution,io_gather,isvalgrind,isgprof,isdakota,isoceancoupling) % {{{
			...
			fprintf(fid,'source $ISSM_DIR/scripts/startup.sh\n');
		...
		function LaunchQueueJob(cluster,modelname,dirname,filelist,restart,batch) % {{{
			...
			if ~isempty(restart)
				...
				launchcommand=['! cd ' cluster.executionpath ' && rm -rf ./' dirname ' && mkdir ' dirname ...
					' && cd ' dirname ' && mv ' pth_data '/' dirname '.tar.gz ./ && tar -zxf ' dirname '.tar.gz  && hostname && qsub ' modelname '.queue '];
			end
            disp(launchcommand);
            eval(launchcommand);
		...
		function Download(cluster,dirname,filelist) % {{{
            ...
            disp('we are in download');
            issmscpin_ongadi(cluster.name,cluster.login,cluster.port,directory,filelist);

		end %}}}
		function SaveJobFromScratch(cluster,modelname,dirname) % {{{
			
			disp('launching solution sequence on remote cluster');
			savecommand=['! cd ' cluster.executionpath ' && cd ' dirname];
            disp('we are in savejob');
            disp(savecommand)
            eval(savecommand);
			% issmssh(cluster.name,cluster.login,cluster.port,savecommand);
		end %}}}
	end
end
