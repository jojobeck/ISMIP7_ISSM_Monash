...
% (proj_run_MRI_ESM2_ssp126_2015_2300_yearly.m).
%
% NEW FILE -- neither the original proj_run_MRI_ESM2_ssp126_2015_2300.m
...
%                                 original used
%                                 (postprocessed_data/MRI-ESM2/ssp126/)
...
%                                 ssp126/C007 submission going forward.
%                                 The original run's own NetCDF output was
%                                 moved to
%                                 postprocessed_data/MRI-ESM2/ssp126_UnconstrainedCollapse/
%                                 (matching this project's existing
%                                 ssp126_test / ssp126_dT0 sibling-naming
%                                 convention) before this script's step 2
%                                 was first run, so writing into ssp126/
...
%                                 same technique as
%                                 plot_calvingfront_model_ssp126.m's own
...
% error.
%
% Usage (run from proj_runs/MRI-ESM2/ssp126/, ISSM already on the
...
%   analyze_proj_ssp126_yearly([1 2 3])   % all three (default)
    ...
    if perform(org, 'WriteISMIP6_NetCDF_yearly') % {{{
        ...
        % output (postprocessed_data/MRI-ESM2/ssp126/, experiment_id
        % 'ssp126', set_counter 'C007') -- deliberately substituting that
        ...
        meta.set_counter        = 'C006';     % same as the original run
    ...
    if perform(org, 'CalvingFrontEvolution_yearly') % {{{
        ...
        % plot_calvingfront_model_ssp126.m's own plot_calvingfront_evolution
