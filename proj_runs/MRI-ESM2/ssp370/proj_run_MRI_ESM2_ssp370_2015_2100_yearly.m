function md = proj_run_MRI_ESM2_ssp370_2015_2100_yearly(steps, start_year, end_year, waitonlock_seconds)
% Standalone yearly-restart variant of proj_run_MRI_ESM2_ssp370_2015_2100.m.
%
...
%                          step.
%   3  ProjLevelset -- build the cumulative collapse-mask spclevelset,
...
% the ORIGINAL script's own end_year=2100 meaning exactly (there,
% final_time=end_year+1 produces the calendar-year-end_year output
% snapshot). A full run therefore still uses end_year=2100, producing 286
% iterations (2015 through 2100 inclusive) -- the same total simulated
...
%   proj_run_MRI_ESM2_ssp370_2015_2100_yearly([1])                  % build TF only
%   proj_run_MRI_ESM2_ssp370_2015_2100_yearly([2])                  % build SMB only
%   proj_run_MRI_ESM2_ssp370_2015_2100_yearly([3])                  % build v2 levelset only
%   proj_run_MRI_ESM2_ssp370_2015_2100_yearly([1 2 3])              % build all three
%   proj_run_MRI_ESM2_ssp370_2015_2100_yearly([4], 2015, 2015, 3600)   % 1-yr test, 1 hr timeout
%   proj_run_MRI_ESM2_ssp370_2015_2100_yearly([4], 2015, 2100, 6*3600) % full run, 6 hr timeout/yr
%   proj_run_MRI_ESM2_ssp370_2015_2100_yearly([4], 2100, 2100, 6*3600) % resume after a crash at 2100
%
...
%     addpath('$ISSM_DIR/lib'); proj_run_MRI_ESM2_ssp370_2015_2100_yearly([4],2015,2015,3600), quit"
    ...
    if perform(org, 'ProjTF') % {{{
        ...
        save([preproc_proj_ocean 'MRI_ESM2_TF_' SCENARIO '_' ...
    ...
    if perform(org, 'ProjSMB') % {{{
        ...
        nc_first = [smb_ssp_dir sprintf('acabf_AIS_%s_%s_SDBN1-2000m_v1_%d.nc', CMIP_MODEL, SCENARIO, build_start_year)];
        ...
        for k = 1:nyears_max
            ...
            nc_smb  = [smb_ssp_dir sprintf('acabf_AIS_%s_%s_SDBN1-2000m_v1_%d.nc', CMIP_MODEL, SCENARIO, yr)];
            nc_grad = [grad_ssp_dir sprintf('dacabfdz_AIS_%s_%s_SDBN1-2000m_v1_%d.nc', CMIP_MODEL, SCENARIO, yr)];
        ...
        save([preproc_proj_atmo 'MRI_ESM2_SMB_' SCENARIO '_' ...
              num2str(build_start_year) '_' num2str(build_end_year) '.mat'], ...
    ...
    if perform(org, 'ProjLevelsetV1') % {{{
        ...
        % build_and_compare_levelset_v2_ssp370.m, copied here (see that
    ...
    if perform(org, 'ProjRunYearly') % {{{
        ...
        load([preproc_proj_ocean 'MRI_ESM2_TF_' SCENARIO '_2015_2100.mat']);         % -> tf_proj, z_data
        load([preproc_proj_atmo  'MRI_ESM2_SMB_' SCENARIO '_2015_2100.mat']);        % -> smb_forcing, bgrad_forcing
        ...
        loaded_lset = load([preproc_proj_ocean 'MRI_ESM2_levelset_v1_' SCENARIO '_2015_2100.mat']);   % -> proj_spclevelset_v2
        ...
        %   1. the solved model AIS_ISMIP7_Proj_ssp370_yearly_<start_year>.mat
        ...
        %      AIS_ISMIP7_Proj_ssp370_yearly_appliedcollapse_<start_year>.mat.
        ...
        % simulated, matching the ORIGINAL script's own end_year=2100
        ...
        % -- a full run needs end_year=2100 to reproduce the original's
        ...
        for year = start_year:end_year
            ...
            % data only exists through 2299 (decade chunks 2015-2299, no 2100
            ...
            % (per-year acabf/dacabfdz files can also run out before 2100 --
...
% Copied verbatim from build_and_compare_levelset_v2_ssp370.m's own
