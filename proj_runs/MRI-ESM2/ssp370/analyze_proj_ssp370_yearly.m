function md = analyze_proj_ssp370_yearly(steps)
% Post-processing / output-analysis for the yearly-restart ssp370 run
% (proj_run_MRI_ESM2_ssp370_2015_2100_yearly.m).
%
% Mirrors analyze_proj_ssp585_yearly.m / analyze_proj_ssp126_yearly.m
% exactly -- see those files' own comments for the full rationale, not
% repeated in full here. NEW FILE -- old_scripts/proj_run_MRI_ESM2_ssp370_2015_2100.m
% is NOT modified.
%
% Each step below loads ONLY what it needs (proj_runs/functions/), not a
% single shared up-front load:
...
%     only 2 scalars per year, not the full per-year field set. Filters
%     out the sub-annual "safety step" entry per file (keep = near-integer
%     time only), so an N-year run returns N points here, not 2N.
...
%     the sparse years (every 30, plus the final year -- for an 86-year
%     ssp370 run that's ~4 snapshots) it actually plots, not the whole run.
% Fully scenario-agnostic (SCENARIO passed straight through), discovers
% whatever Models_yearly/*.mat files actually exist rather than assuming a
% fixed start/end year -- this scenario's own run only covers 2015-2100
% (86 years, not 286), and nothing here assumes otherwise (see step 2's
% time_range, computed from the actual data rather than hardcoded).
%
% Step map:
%   1  VAFCheck_yearly           compute the VAF/SLE curve (identical
%                                 formula to the original's step 5
%                                 VAPlotCheck), save figure to
...
%                                 by the original script's step 7 --
%                                 output goes to the SAME directory the
%                                 original used
%                                 (postprocessed_data/MRI-ESM2/ssp370/)
%                                 using experiment_id='ssp370' and
%                                 set_counter='C003' -- SAME labels as the
%                                 original (ungated) run, deliberately: the
%                                 yearly floating-gated run is intended to
%                                 SUBSTITUTE that data as the real
%                                 ssp370/C003 submission going forward.
%                                 The original run's own NetCDF output was
%                                 moved to
%                                 postprocessed_data/MRI-ESM2/ssp370_UnconstrainedCollapse/
%                                 before this script's step 2 is first run,
%                                 so writing into ssp370/ here does not
%                                 destroy that prior data -- it's preserved
%                                 under the renamed path.
%   3  CalvingFrontEvolution_yearly  mesh-native calving-front isoline
%                                 (MaskIceLevelset==0, via isoline()),
%                                 every 30 years across the whole run
%                                 (plus the final available year, even if
%                                 it doesn't fall on an exact 30-year mark
%                                 from the first year), colour-coded by
%                                 year over the earliest-loaded-year ice
%                                 mask background. Saves
...
% error.
%
% Usage (run from proj_runs/MRI-ESM2/ssp370/, ISSM already on the
% MATLAB path -- pure post-processing, no PBS submission/waitonlock
% involved at all):
%   analyze_proj_ssp370_yearly([1])       % VAF check only (fast)
%   analyze_proj_ssp370_yearly([2])       % NetCDF write only (slow -- needs every field/year)
%   analyze_proj_ssp370_yearly([3])       % calving-front evolution plot only (fast)
%   analyze_proj_ssp370_yearly([1 2 3])   % all three (default)
    ...
    if perform(org, 'VAFCheck_yearly') % {{{
        ...
        % TransientSolution this step doesn't need, and filters out the
        % sub-annual safety step per file. Identical VAF/SLE formula to
        % the original script's step 5 (VAPlotCheck).
    ...
    if perform(org, 'WriteISMIP6_NetCDF_yearly') % {{{
        ...
        % load, done only here.
        %
        % Identical to the original script's step 7 (WriteISMIP6_NetCDF):
        ...
        % whatever sub-annual "safety step" entries ISSM may add.
        %
        % Writes into the SAME path/labels as the original script's own
        % output (postprocessed_data/MRI-ESM2/ssp370/, experiment_id
        % 'ssp370', set_counter 'C003') -- deliberately substituting that
        % data, per instruction. The original run's own output was already
        % moved to ssp370_UnconstrainedCollapse/ (see module docstring)
        % before this was first run, so nothing is destroyed -- but
        % running this again WILL overwrite whatever is currently in
        % ssp370/, including results from any earlier run of this same
        % yearly script.
        md = load_and_concatenate_yearly_models(modeldir, SCENARIO);

        outdir = [proj_root 'postprocessed_data/MRI-ESM2-0/' SCENARIO '/'];
        ...
        % global attribute, so a hardcoded string would be wrong for any
        % run that doesn't cover the full nominal span. Same
        % filter/rounding convention those functions already use
        % internally.
        ...
        meta                    = struct();
        meta.experiment_id      = SCENARIO;   % 'ssp370' -- same as the original run
        meta.set_counter        = 'C004';     % same as the original run
    ...
    if perform(org, 'CalvingFrontEvolution_yearly') % {{{
        ...
        % mask. Same isoline() technique used across this project
