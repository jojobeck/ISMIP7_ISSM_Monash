function md = analyze_proj_ssp126_yearly(steps)
% Post-processing / output-analysis for the yearly-restart ssp126 run
% (proj_run_CESM_WACCM_ssp126_2015_2300_yearly.m).
%
% Mirrors analyze_proj_ssp585_yearly.m exactly -- see that file's own
% comments for the full rationale, not repeated in full here. NEW FILE --
% neither the original proj_run_CESM_WACCM_ssp126_2015_2300.m nor the
% yearly run script itself are modified.
%
...
% fixed start/end year, so this works whether this particular scenario's
% run covers the full 2015-2300 span or stops earlier (e.g. 2100).
% Because each step is now independent, running e.g. just [1] or just [3]
% no longer pays the cost of the full multi-hundred-file concatenation
% that WriteISMIP6_NetCDF_yearly alone actually needs.
%
% Step map:
...
%                                 original used
%                                 (postprocessed_data/CESM2-WACCM/ssp126/)
%                                 using experiment_id='ssp126' and
%                                 set_counter='C005' -- SAME labels as the
%                                 original (ungated) run, deliberately: the
%                                 yearly floating-gated run is intended to
%                                 SUBSTITUTE that data as the real
%                                 ssp126/C005 submission going forward.
%                                 The original run's own NetCDF output was
%                                 moved to
%                                 postprocessed_data/CESM2-WACCM/ssp126_UnconstrainedCollapse/
%                                 before this script's step 2 is first run,
%                                 so writing into ssp126/ here does not
...
% error.
%
% Usage (run from proj_runs/CESM2-WACCM/ssp126/, ISSM already on the
% MATLAB path -- pure post-processing, no PBS submission/waitonlock
% involved at all):
%   analyze_proj_ssp126_yearly([1])       % VAF check only (fast)
%   analyze_proj_ssp126_yearly([2])       % NetCDF write only (slow -- needs every field/year)
%   analyze_proj_ssp126_yearly([3])       % calving-front evolution plot only (fast)
%   analyze_proj_ssp126_yearly([1 2 3])   % all three (default)
    ...
    if perform(org, 'WriteISMIP6_NetCDF_yearly') % {{{
        ...
        % output (postprocessed_data/CESM2-WACCM/ssp126/, experiment_id
        % 'ssp126', set_counter 'C005') -- deliberately substituting that
        % data, per instruction. The original run's own output was already
        % moved to ssp126_UnconstrainedCollapse/ (see module docstring)
        % before this was first run, so nothing is destroyed -- but
        % running this again WILL overwrite whatever is currently in
        % ssp126/, including results from any earlier run of this same
        % yearly script.
        ...
        % any run that doesn't cover the full span (e.g. if this scenario
        % ends up only run through some earlier year). Same
        ...
        meta                    = struct();
        meta.experiment_id      = SCENARIO;   % 'ssp126' -- same as the original run
        meta.set_counter        = 'C005';     % same as the original run
