function md = proj_run_CESM_WACCM_ssp126_2015_2300_yearly(steps, start_year, end_year, waitonlock_seconds)
% Standalone yearly-restart variant of proj_run_CESM_WACCM_ssp126_2015_2300.m.
% Mirrors proj_run_CESM_WACCM_ssp585_2015_2300_yearly.m exactly (same
% floating-only gating mechanism, same 2015-no-advance seed, same
% migration_max, same Models_yearly/ storage) -- see that file's own
% header comments for the full rationale, not repeated in full here.
%
% NEW FILE -- the original script (proj_run_CESM_WACCM_ssp126_2015_2300.m)
% is NOT modified. This script builds all of its own inputs and does not
...
%                          from the original ssp126 script's own ProjTF step.
%   2  ProjSMB          -- build annual SMB+gradient .mat 2015-2300, copied
%                          verbatim from the original ssp126 script's own
%                          ProjSMB step.
...
%                          ssp585 yearly script's step 4, loading the
%                          v2.1 levelset (step 3's output here) instead
%                          of v2.
%
% WHY v2.1 (not v2) FOR THE COLLAPSE MASK, UNLIKE ssp585:
%   ssp126's raw_data has BOTH fracture/v2/ and fracture/v2.1/ (ssp585 only
%   has v1/ and v2/ -- no v2.1 exists there). v2.1's own README top-line
%   title text incorrectly says "SSP5-3.4-over Forcing" (a different
%   scenario) despite the file being under the ssp126/ directory tree with
%   ssp126 in every filename -- this was flagged and investigated directly
%   with the user rather than assumed. The NetCDF's own global attributes
%   (not just the README free text) settle it:
%     v2:   :Date = "25-Jun-2026", :info2 = "...cesm2waccm_ssp126_ismip7_8km-v2"
%     v2.1: :Date = "22-Jul-2026", :info2 = "...cesm2waccm_ssp126_ismip7_8km-v2.1"
%   Both explicitly reference ssp126 internally; identical grid/dims
%   (761x761x350). v2.1 is confirmed a genuine, ~1-month-newer regeneration
%   of the SAME ssp126 collapse mask (the README title is simply a
%   copy-paste error from another scenario's template), so v2.1 -- being
%   the newer, presumably corrected version -- is used here. Confirmed
%   with the user before writing this step.
%
...
% crash at 2100: rerun with start_year=2100) -- restores both the solved
...
%   proj_run_CESM_WACCM_ssp126_2015_2300_yearly([1])                  % build TF only
%   proj_run_CESM_WACCM_ssp126_2015_2300_yearly([2])                  % build SMB only
%   proj_run_CESM_WACCM_ssp126_2015_2300_yearly([3])                  % build v2.1 levelset only
%   proj_run_CESM_WACCM_ssp126_2015_2300_yearly([1 2 3])              % build all three
%   proj_run_CESM_WACCM_ssp126_2015_2300_yearly([4], 2015, 2015, 3600)   % 1-yr test, 1 hr timeout
%   proj_run_CESM_WACCM_ssp126_2015_2300_yearly([4], 2015, 2300, 6*3600) % full run, 6 hr timeout/yr
%   proj_run_CESM_WACCM_ssp126_2015_2300_yearly([4], 2100, 2300, 6*3600) % resume after a crash at 2100
%
...
%     addpath('$ISSM_DIR/lib'); proj_run_CESM_WACCM_ssp126_2015_2300_yearly([4],2015,2015,3600), quit"
    ...
    % original ssp126 script's own header, read fresh from source before
    ...
    if perform(org, 'ProjTF') % {{{
        % Copied verbatim from the original ssp126 script's own ProjTF
    ...
    if perform(org, 'ProjSMB') % {{{
        % Copied verbatim from the original ssp126 script's own ProjSMB
        % step (read fresh from source before writing this).
        md = loadmodel(inputmodel_2015);
    ...
    if perform(org, 'ProjLevelsetV21') % {{{
        ...
        % ssp585, pointed at the ssp126 v2.1 collapse-mask NetCDF (see
        % header comment for why v2.1, not v2).
    ...
    if perform(org, 'ProjRunYearly') % {{{
        % The annual loop -- identical logic to the ssp585 yearly script's
        % step 4 (see that file's header for full rationale on the
        % floating-only gating, 2015 seed, and migration_max), loading the
        % v2.1 levelset (step 3's output here) instead of v2.
        fprintf('=== Step 4: annual loop, %d -> %d ===\n', start_year, end_year);

        % Same requested_outputs list as the original ssp126 script.
        ismip6_outputs = { ...
            'default', ...
        ...
        loaded_lset = load([preproc_proj_ocean 'CESM_WACCM_levelset_v21_' SCENARIO '_2015_2300.mat']);   % -> proj_spclevelset_v21
        ...
        for year = start_year:end_year
            ...
            md.levelset.migration_max      = 2863.78;   % m/yr -- same Greene-calibrated cap as ssp585
