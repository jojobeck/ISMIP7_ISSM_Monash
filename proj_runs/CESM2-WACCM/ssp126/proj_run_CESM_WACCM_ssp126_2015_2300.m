function md = proj_run_CESM_WACCM_ssp126_2015_2300(steps, loadonly)
% Projection run 2015-2300 using CESM2-WACCM ssp126 forcing.
%
...
%   Levelset   : fracture/v2/ice_shelf_collapse_mask_<model>_<scenario>_ismip7_8km-v2.nc
%                Annual collapse flag (0=sustainable, 1=collapsed); made cumulative
%                here so once a shelf collapses it stays constrained.
%                NOTE: unlike the ssp585 script (which uses the older,
%                unversioned fracture/ file), ssp126's raw_data only has the
%                fracture/v2/ file -- confirmed on disk, there is no
%                unversioned or v1 equivalent for this scenario.
...
%                        postprocessed_data/figures/CESM2-WACCM/ssp126/
%   8  WriteISMIP6_NetCDF_test writes 4 variables to ssp126_test/ for quick
...
%                        postprocessed_data/CESM2-WACCM/ssp126/  (EXP='C005')

    % ---- swappable scenario label ----------------------------------------
    SCENARIO   = 'ssp126';   % ssp126 | ssp370 | ssp534-over | ssp585
    ...
    end_year    = 2300;   % last year with available forcing; final snapshot
                          ...
                          % (ISMIP7 ssp126 requires 2015-2300 = 286 years)
    ...
    if perform(org, 'ProjTF') % {{{
        ...
        % the last real chunk actually covers. Do NOT fabricate a padded/
        % repeated entry beyond that: the array is saved exactly as long as
        % the real data. The transient-run steps run past this array's end
        % anyway (final_time = end_year+1); with interp_forcing=0
        % (step-function forcing), ISSM holds the last supplied year's TF
        % constant for any later time, so no explicit padding is needed here.
    ...
    if perform(org, 'ProjSMB') % {{{
        ...
        % confirmed for ssp585's year 2300 dacabfdz file via ncdump; check
        % applied here too in case ssp126 has the same tail gap) rather than
        % being simply absent like TF's decade chunks, so each year's 'time'
        % coordinate is checked (cheap 1D read) before reading/interpolating
        % the full spatial field. Do NOT fabricate a padded/repeated entry
        % for a missing year: the array is saved exactly as long as the real
        % data. The transient-run steps run past this array's end anyway;
        % with interp_forcing=0 (step-function forcing), ISSM holds the last
        % supplied year's SMB constant for any later time, so no explicit
    ...
    if perform(org, 'WriteISMIP6_NetCDF_test') % {{{
        ...
        % Output goes to postprocessed_data/CESM2-WACCM/ssp126_test/
        addpath('./../../functions');
        ...
        outdir_test = [proj_root 'postprocessed_data/CESM2-WACCM/ssp126_test/'];
        if ~exist(outdir_test, 'dir'), mkdir(outdir_test); end

        meta_t                    = struct();
        meta_t.experiment_id      = SCENARIO;
        meta_t.set_counter        = 'C005';
    ...
    if perform(org, 'WriteISMIP6_NetCDF') % {{{
        ...
        meta                    = struct();
        meta.experiment_id      = SCENARIO;        % 'ssp126'
        meta.set_counter        = 'C005';
