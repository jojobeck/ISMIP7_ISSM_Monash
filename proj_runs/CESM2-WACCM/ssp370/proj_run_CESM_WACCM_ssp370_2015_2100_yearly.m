function md = proj_run_CESM_WACCM_ssp370_2015_2100_yearly(steps, start_year, end_year, waitonlock_seconds)
% Standalone yearly-restart variant of old_scripts/proj_run_CESM_WACCM_ssp370_2015_2100.m.
% Mirrors proj_run_CESM_WACCM_ssp585_2015_2300_yearly.m / ssp126's own
% yearly script exactly (same floating-only gating mechanism, same
% 2015-no-advance seed, same migration_max, same Models_yearly/ storage)
% -- see the ssp585 file's own header comments for the full rationale, not
% repeated in full here. The ONE substantive difference: ssp370 only has
% real forcing data through 2100 (confirmed on disk -- TF's last decade
% chunk is 2095-2100.nc, SMB's last annual file is year 2100, no 2101+
% files exist at all for this scenario), so end_year defaults to 2100
% here, not 2300.
%
% NEW FILE -- old_scripts/proj_run_CESM_WACCM_ssp370_2015_2100.m is NOT
% modified (moved there unmodified, not edited). This script builds all
% of its own inputs and does not require that original script to have
% been run first.
%
% Step map (steps=[4] by default -- i.e. assumes 1-3 already run/available):
%   1  ProjTF           -- build annual TF .mat 2015-2100, copied verbatim
%                          from the original ssp370 script's own ProjTF step.
%   2  ProjSMB          -- build annual SMB+gradient .mat 2015-2100, copied
%                          verbatim from the original ssp370 script's own
%                          ProjSMB step.
%   3  ProjLevelsetV21  -- build the cumulative collapse-mask spclevelset
%                          from the fracture/v2.1/ collapse-mask NetCDF
%                          (NOT v2 -- see below), using the same
%                          build_spclevelset() logic already verified for
%                          ssp585/ssp126.
%   4  ProjRunYearly    -- the annual loop -- identical logic to the
%                          ssp585/ssp126 yearly scripts' step 4, loading
%                          the v2.1 levelset (step 3's output here).
%
% WHY v2.1 (not v2) FOR THE COLLAPSE MASK, MATCHING ssp126's DECISION:
%   ssp370's raw_data has BOTH fracture/v2/ and fracture/v2.1/ (same as
%   ssp126; ssp585 only has v1/ and v2/ -- no v2.1 exists there). v2.1's
%   own README top-line title text incorrectly says "SSP5-3.4-over
%   Forcing" (a different scenario) despite the file being under the
%   ssp370/ directory tree with ssp370 in every filename -- the SAME
%   batch copy-paste labelling error already found and investigated for
%   ssp126. The NetCDF's own global attributes (not just the README free
%   text) settle it:
%     v2:   :Date = "25-Jun-2026", :info2 = "...cesm2waccm_ssp370_ismip7_8km-v2"
%     v2.1: :Date = "22-Jul-2026", :info2 = "...cesm2waccm_ssp370_ismip7_8km-v2.1"
%   Both explicitly reference ssp370 internally. v2.1 is confirmed a
%   genuine, ~1-month-newer regeneration of the SAME ssp370 collapse mask
%   (the README title is simply a copy-paste error from another
%   scenario's template, same as ssp126's), so v2.1 -- being the newer,
%   presumably corrected version -- is used here, matching the precedent
%   set for ssp126. Confirmed with the user before writing this step.
%
% Steps 1-3 ALWAYS build the FULL 2015-2100 span regardless of what
...
% crash at 2080: rerun with start_year=2080) -- restores both the solved
% model and applied_collapse from what a prior run already saved.
%
% Usage (start_year/end_year/waitonlock_seconds default to 2015/2100/3600):
%   proj_run_CESM_WACCM_ssp370_2015_2100_yearly([1])                  % build TF only
%   proj_run_CESM_WACCM_ssp370_2015_2100_yearly([2])                  % build SMB only
%   proj_run_CESM_WACCM_ssp370_2015_2100_yearly([3])                  % build v2.1 levelset only
%   proj_run_CESM_WACCM_ssp370_2015_2100_yearly([1 2 3])              % build all three
%   proj_run_CESM_WACCM_ssp370_2015_2100_yearly([4], 2015, 2015, 3600)   % 1-yr test, 1 hr timeout
%   proj_run_CESM_WACCM_ssp370_2015_2100_yearly([4], 2015, 2100, 6*3600) % full run, 6 hr timeout/yr
%   proj_run_CESM_WACCM_ssp370_2015_2100_yearly([4], 2080, 2100, 6*3600) % resume after a crash at 2080
%
...
%     addpath('$ISSM_DIR/lib'); proj_run_CESM_WACCM_ssp370_2015_2100_yearly([4],2015,2015,3600), quit"
    ...
    % original ssp370 script's own header, read fresh from source before
    % writing this, except collapse_nc_v21 which points at fracture/v2.1/
    ...
    collapse_nc_v21 = [raw_ssp 'fracture/v2.1/ice_shelf_collapse_mask_' ...
                       lower(strrep(CMIP_MODEL,'-','')) '_' SCENARIO '_ismip7_8km-v2.1.nc'];
    ...
    % Build steps (1-3) always cover the FULL 2015-2100 span, independent
    ...
    if perform(org, 'ProjTF') % {{{
        % Copied verbatim from the original ssp370 script's own ProjTF
        % step (read fresh from source before writing this).
    ...
    if perform(org, 'ProjSMB') % {{{
        % Copied verbatim from the original ssp370 script's own ProjSMB
        % step (read fresh from source before writing this).
        md = loadmodel(inputmodel_2015);
    ...
    % ================================================================= Step 3
    if perform(org, 'ProjLevelsetV21') % {{{
        % Same build_spclevelset() logic already written and verified for
        % ssp585/ssp126, pointed at the ssp370 v2.1 collapse-mask NetCDF
        % (see header comment for why v2.1, not v2).
        proj_spclevelset_v21 = build_spclevelset(inputmodel_2015, collapse_nc_v21, build_start_year, build_end_year);
        v21_fname = [preproc_proj_ocean 'CESM_WACCM_levelset_v21_' SCENARIO '_' ...
                    num2str(build_start_year) '_' num2str(build_end_year) '.mat'];
        save(v21_fname, 'proj_spclevelset_v21', '-v7.3');
        fprintf('Saved: %s\n', v21_fname);
    end % }}}

    % ================================================================= Step 4
    if perform(org, 'ProjRunYearly') % {{{
        % The annual loop -- identical logic to the ssp585/ssp126 yearly
        % scripts' step 4 (see ssp585's header for full rationale on the
        % floating-only gating, 2015 seed, and migration_max), loading the
        % v2.1 levelset (step 3's output here). end_year defaults to 2100
        % (not 2300) -- see module docstring for why.
        fprintf('=== Step 4: annual loop, %d -> %d ===\n', start_year, end_year);

        % Same requested_outputs list as the original ssp370 script.
        ismip6_outputs = { ...
            'default', ...
        ...
        load([preproc_proj_ocean 'CESM_WACCM_TF_' SCENARIO '_2015_2100.mat']);         % -> tf_proj, z_data
        load([preproc_proj_atmo  'CESM_WACCM_SMB_' SCENARIO '_2015_2100.mat']);        % -> smb_forcing, bgrad_forcing

        % v2.1 levelset (step 3's output) -- aliased to proj_spclevelset so
        % the rest of this loop's logic (unchanged from ssp585's) doesn't
        % need to reference a different variable name.
        loaded_lset = load([preproc_proj_ocean 'CESM_WACCM_levelset_v21_' SCENARIO '_2015_2100.mat']);   % -> proj_spclevelset_v21
        proj_spclevelset = loaded_lset.proj_spclevelset_v21;
        clear loaded_lset;
        ...
        % ---- Restart state (see ssp585 yearly script's header/in-loop
        % comments for full rationale -- identical mechanism here). ----
        if start_year == 2015
            md = loadmodel(inputmodel_2015);
        ...
        % end_year is INCLUSIVE (see ssp585 yearly script's header comment
        % for the full explanation of why -- identical convention here,
        % just with a 2100 ceiling instead of 2300).
        for year = start_year:end_year
            ...
            % (backward-looking "most recent at-or-before year" tag -- see
            % ssp585 yearly script's in-loop comment for the full
            % step-function-forcing rationale). ---
            tf_year = tf_proj;
            ...
            % --- Calving front: collapse-mask forcing GATED to floating
            % ice only, using the model's OWN current simulated state --
            % see ssp585 yearly script's in-loop comment for the full
            % mechanism and why the gating decision must persist
            % (applied_collapse) rather than being re-evaluated fresh
            % every year.
            ...
            md.levelset.migration_max      = 2863.78;   % m/yr -- same Greene-calibrated cap as ssp585/ssp126
...
% Copied verbatim from proj_run_CESM_WACCM_ssp585_2015_2300_yearly.m's own
% build_spclevelset() (itself copied verbatim from the original scripts'
% ProjLevelset step) -- only collapse_nc is parameterised, so this same
% logic can be pointed at v1, v2, or v2.1 data. See that step's own
% comments for the full rationale (cumulative collapse, advance-suppression).
