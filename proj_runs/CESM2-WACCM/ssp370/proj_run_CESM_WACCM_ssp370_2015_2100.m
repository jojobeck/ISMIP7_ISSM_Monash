function md = proj_run_CESM_WACCM_ssp370_2015_2100(steps, loadonly)
% Projection run 2015-2100 using CESM2-WACCM ssp370 forcing.
%
% Unlike ssp126/ssp585 (2015-2300, 286 years, two-segment restart), ssp370
% only requires output through 2100 -- confirmed against the raw data: the
% last real TF decade chunk is 2095-2100.nc and the last real acabf/dacabfdz
% file is year 2100 (no 2101+ files exist at all for this scenario). So the
% whole 86-year run (2015-2100) is a single continuous transient (step 4) --
% no restart split is needed the way ssp126/ssp585 need one at 2151.
%
% To run a different emission scenario, copy this file, rename the function
% and file to match:
%   ssp126 | ssp370 | ssp534-over | ssp585
%
% Forcing sources (all under raw_data/ISMIP7/AIS/CESM2-WACCM/ssp370/):
%   Ocean TF   : ocean/tf/v3/  (decade NetCDF chunks 2015-2100, last chunk
%                2095-2100.nc; ISSM holds the last value for any later time
%                since interp_forcing=0, though final_time never exceeds 2100
%                here anyway)
%   SMB        : SDBN1-2000m/acabf/v2/    (annual NetCDF, kg m-2 s-1, 2015-2100)
%   SMB grad   : SDBN1-2000m/dacabfdz/v2/ (annual NetCDF, kg m-2 s-1 m-1, 2015-2100)
%   Levelset   : fracture/v2/ice_shelf_collapse_mask_<model>_ssp370_ismip7_8km-v2.nc
%                Annual collapse flag (0=sustainable, 1=collapsed); made cumulative
%                here so once a shelf collapses it stays constrained.
%                NOTE: like ssp126 (and unlike ssp585's older, unversioned
%                fracture/ file), ssp370's raw_data only has the fracture/v2/
%                file -- confirmed on disk, there is no unversioned or v1
...
%
% Step map:
%   1  ProjTF            build annual TF mat 2015-2100, save to
%                        preprocessed_data/Ocean/Proj/CESM2-WACCM/ssp370/
%   2  ProjSMB           build annual SMB mat 2015-2100 (CESM anomaly
%                        relative to 1995-2014 historical mean + RACMO clim),
%                        save to preprocessed_data/Atmosphere/Proj/CESM2-WACCM/ssp370/
%   3  ProjLevelset      build cumulative ice-shelf-collapse spclevelset 2015-2100,
%                        save to preprocessed_data/Ocean/Proj/CESM2-WACCM/ssp370/
%   4  ProjRun_2015_2100 single continuous transient 2015-2100 starting from
%                        AIS_state_2015 (loadonly=0 submit, =1 gather)
%   5  WriteISMIP6_NetCDF_test writes 4 variables to ssp370_test/ for quick
%                        compliance checking (sftgif, iareagr, tendlibmassbfgr,
%                        dlithkdt)
%   6  WriteISMIP6_NetCDF grids the TransientSolution onto the ISMIP6 8 km
%                        AIS grid and writes one NetCDF per Appendix-2
%                        variable into postprocessed_data/CESM2-WACCM/ssp370/
%                        (EXP='C003')

    % ---- swappable scenario label ----------------------------------------
    SCENARIO   = 'ssp370';   % ssp126 | ssp370 | ssp534-over | ssp585
    ...
    collapse_nc = [raw_ssp  'fracture/v2/ice_shelf_collapse_mask_' ...
    ...
    % ------------------------------------------------------------------ time
    start_year  = 2015;
    end_year    = 2100;   % last year with available forcing (confirmed: no
                          % 2101+ TF/acabf/dacabfdz files exist for ssp370);
                          % final snapshot lands at t=end_year+1=2101 →
                          % nominal year 2100 (ISMIP7 ssp370 requires
                          % 2015-2100 = 86 years)
    ...
    if perform(org, 'ProjTF') % {{{
        ...
        % the last real chunk actually covers (expected: 2095-2100.nc, i.e.
        % last_real_yr == end_year == 2100 for this scenario -- but the same
        % trim-to-last-real-year logic as ssp126/ssp585 is kept here in case
        % a future data update leaves a shorter tail). Do NOT fabricate a
        % padded/repeated entry beyond the real data.
    ...
    if perform(org, 'ProjSMB') % {{{
        ...
        % actually contain real data (expected: 2100, no 2101+ files exist
        % for this scenario). Per-year files can exist as empty placeholders
        % (time dimension UNLIMITED, 0 records currently -- confirmed for
        % ssp585's year 2300 dacabfdz file via ncdump; the same check is
        % applied here for robustness) rather than being simply absent like
        % TF's decade chunks, so each year's 'time' coordinate is checked
        % (cheap 1D read) before reading/interpolating the full spatial field.
        % Do NOT fabricate a padded/repeated entry for a missing year.
        %
    ...
    if perform(org, 'ProjLevelset') % {{{
        ...
        for k = 1:length(model_years)
            ...
            % Find closest available year in the mask file
    ...
    % ================================================================= Step 4
    if perform(org, 'ProjRun_2015_2100') % {{{
        % Single continuous transient 2015-2100 (86 years -- short enough
        % that, unlike ssp126/ssp585's 286-year runs, no restart split at
        % 2151 is needed here). Starting from the AIS_state_2015 end-state
        % of the historical run (hist_run_CESM_WACCM_1995_2014 step 3).
        ...
        md.timestepping.final_time = end_year + 1;   % last snapshot at 2100
    ...
    if perform(org, 'VAPlotCheck') % {{{
        ...
        md1 = loadmodel(org, 'ProjRun_2015_2100');
        ...
        title(sprintf('%s %s — VAF SLE', CMIP_MODEL,SCENARIO), ...
              'Interpreter', 'none');
        grid on;
    ...
    if perform(org, 'WriteISMIP6_NetCDF_test') % {{{
        ...
        % Output goes to postprocessed_data/CESM2-WACCM/ssp370_test/
        addpath('./../../functions');

        md = loadmodel(org, 'ProjRun_2015_2100');

        outdir_test = [proj_root 'postprocessed_data/CESM2-WACCM/ssp370_test/'];
        if ~exist(outdir_test, 'dir'), mkdir(outdir_test); end

        meta_t                    = struct();
        meta_t.experiment_id      = SCENARIO;
        meta_t.set_counter        = 'C003';
        meta_t.time_range         = '2015-2100';
        ...
        nT          = sum(keep);
        t_annual     = round(t_raw(keep));   % ISSM time [2016, ..., 2100]
        time_yr      = t_annual - 1;         % calendar year [2015, ..., 2099]
    ...
    if perform(org, 'WriteISMIP6_NetCDF') % {{{
        % Grids the TransientSolution (step 4) onto the standard ISMIP7
        ...
        meta                    = struct();
        meta.experiment_id      = SCENARIO;        % 'ssp370'
        meta.set_counter        = 'C003';
        meta.time_range         = '2015-2100';
