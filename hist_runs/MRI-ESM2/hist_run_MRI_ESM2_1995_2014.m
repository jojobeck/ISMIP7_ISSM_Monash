function md = hist_run_MRI_ESM2_1995_2014(steps, loadonly)
% Historical validation run 1995-2014 (CMIP6 "historical" period only, no
% SSP126 extension) using MRI-ESM2 forcing, with the full ISMIP6
...
% artefacts already built by ../../init/hist_run_tune_MRI_ESM2.m for the
% 1995-2020 run and are reused as-is:
%   ../../init/Models/AIS_ISMIP7_Relaxed_MRI_ESM2.mat   (relaxed start state)
%   ../../preprocessed_data/Ocean/Hist/MRI_ESM2_TF_1995_2020.mat
%   ../../preprocessed_data/Atmosphere/Hist/MRI_ESM2_SMB_1995_2020.mat
...
%                              variable into
%                              ../../postprocessed_data/MRI-ESM2/historical/
...
%                              ../../postprocessed_data/MRI-ESM2/historical_test/
...
% and the combined flux equals calving alone.
    ...
    inputmodel_relax_cesm = [init_dir '/Models_MRIESM2/AIS_ISMIP7_Relaxed_MRI_ESM2.mat'];
    ...
    % Forcing .mat files were built for 1995-2020 by init/hist_run_tune_MRI_ESM2.m;
    ...
    if perform(org, 'HistRun_1995_2014') % {{{
        ...
        load([preproc_front 'MRI_ESM2_TF_' num2str(forcing_start_year) '_' num2str(forcing_end_year) '.mat']);
        ...
        load([preproc_atmo 'MRI_ESM2_SMB_corrected_' num2str(forcing_start_year) '_' num2str(forcing_end_year) '.mat']);
        md.smb        = SMBgradients();
    ...
    if perform(org, 'WriteISMIP6_NetCDF') % {{{
        ...
        meta.set_counter        = 'C002';
        meta.time_range         = '1995-2014';
        meta.ESM_id             = 'MRI-ESM2-0';
    ...
    if perform(org, 'WriteISMIP6_NetCDF_test') % {{{
        ...
        outdir = [proj_root 'postprocessed_data/MRI-ESM2/historical_test/'];
        if ~exist(outdir, 'dir'), mkdir(outdir); end
    ...
    if perform(org, 'GLFluxSanityCheck') % {{{
        ...
        tabledir = [proj_root 'postprocessed_data/tables/MRI-ESM2/historical/'];
    ...
    if perform(org, 'GLFluxSanityCheckPlot') % {{{
        ...
        figdir = [proj_root 'postprocessed_data/figures/MRI-ESM2/historical/'];
