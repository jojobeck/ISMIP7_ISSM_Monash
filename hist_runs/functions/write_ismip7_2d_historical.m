function [cfflux_tot, glflux_tot] = write_ismip7_2d_historical(md, outdir, meta)
% WRITE_ISMIP7_2D_HISTORICAL  Grid and write all 2D ISMIP7 NetCDF files for a
%   historical run (e.g. 1995–2014).
%
...
%   flux_along_contour_2d) so write_ismip7_scalar_historical can use them
%   for the IcefrontMassFluxLevelset / GroundinglineMassFlux sign check.
%
...
%     init/scripts/flux_along_contour_2d.m
    ...
    % ---- filter: keep annual outputs only (drop the sub-annual safety step) --
    % The safety step (t≈start+1/12) is excluded by the 0.05 threshold.
    % ISSM convention: t=Y means Jan 1 of year Y = end-of-year-(Y-1) state.
    ...
    % ---- static fields ----------------------------------------------------
    % hfgeoubed: clamp [0, 0.3].  single(0.3) rounds to 0.30000001192... in
    % float64, so subtract one ULP to stay strictly ≤ 0.3.
    ...
    % ---- ice-area fraction fields (ST): clamp [0, 1] ----------------------
    ...
    % τ_b = C² × Neff × |u_b|  (Pa); clamp [0, 3e5] Pa
    % Neff = max(ρ_i·g·H + ρ_w·g·min(bed, 0), 0)  [hydrostatic]
    % Zero under floating ice; uses static md.friction.coefficient.
    rho_w    = md.materials.rho_water;
    C_fric   = md.friction.coefficient;
    ...
    % ---- licalvf / ligroundf / lifmassbf (FL, contour-integral maps) ------
    % ligroundf: flux_along_contour_2d with signed_normal=true returns positive
    % values for the dominant grounded→floating direction.
    % licalvf / lifmassbf: cfflux is unsigned magnitude → negate (mass loss ≤ 0).
    % Clamp limits from CSV: licalvf/lifmassbf [-1e11, 0]; ligroundf [0, 1e11].
